...
export type SeatGridHandle = {
  save: () => void;
  clear: () => void;
  getSelected: () => string[];
};

const SeatGrid = forwardRef<SeatGridHandle, Props>(function SeatGrid(
  ...
  const allBooked = useMemo(() => bookedBeds.length >= 3, [bookedBeds]);
    ...
    <div className="mx-auto w-full max-w-sm">
      <div className="relative aspect-[4/3] w-full rounded-xl border border-slate-300 bg-gradient-to-br from-slate-50 to-slate-100 p-3 shadow-inner">
        <button
          ...
          className={
            "group absolute left-2 top-2 flex h-20 w-28 items-center justify-start rounded-lg border p-2 transition-all " +
            (isBooked("A")
        ...
        <button
          ...
          className={
            "group absolute left-2 bottom-2 flex h-20 w-28 items-center justify-start rounded-lg border p-2 transition-all " +
            (isBooked("B")
              ? "cursor-not-allowed border-red-300 bg-red-50/70 text-red-400"
              : isSelected("B")
              ? "border-emerald-400 bg-emerald-50 shadow ring-2 ring-emerald-300/60"
        ...
        <button
          aria-label="Bed C"
          onClick={() => toggle("C")}
          disabled={isBooked("C") || allBooked}
          className={
            "group absolute right-2 top-1/2 -translate-y-1/2 flex h-28 w-28 items-center justify-start rounded-lg border p-2 transition-all " +
            (isBooked("C")
              ? "cursor-not-allowed border-red-300 bg-red-50/70 text-red-400"
              : isSelected("C")
              ? "border-emerald-400 bg-emerald-50 shadow ring-2 ring-emerald-300/60"
