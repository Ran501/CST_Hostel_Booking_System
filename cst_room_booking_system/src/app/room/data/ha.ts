...
export const HA_NAME = "HA";
export const HA_FLOORS: FloorNumber[] = [1, 2, 3];

export function haAllRoomsForFloor(floor: FloorNumber): number[] {
  const base = floor * 100;
  const count = floor === 1 ? 13 : 14; // Floor 1: 13 rooms, Floors 2-3: 14 rooms
  return Array.from({ length: count }, (_, i) => base + i + 1);
}

export function haLeftRoomsForFloor(floor: FloorNumber): number[] {
  const b = floor * 100;
  if (floor === 1) {
...
export function haRightRoomsForFloor(floor: FloorNumber): number[] {
  const b = floor * 100;
  if (floor === 1) {
...
export function haIsRoomNumberOnFloor(room: number, floor: FloorNumber): boolean {
  const base = floor * 100;
  const max = floor === 1 ? base + 13 : base + 14;
  return room >= base + 1 && room <= max;
}

export function haBookedRoomsForFloor(floor: FloorNumber): number[] {
