...
import ProgressRoomCard from "@/app/room/components/ProgressRoomCard";
...
// Shape of the Neon Database data
interface RoomData {
  roomNumber: string;
  occupied: number;
  capacity: number;
...
interface UserSession {
  ...
  role: string;
  gender: string;
  hasBooked?: boolean; // Added to track booking status
}
    ...
    >
      {isLeftArrow ? (
        <>
          ...
          <span
            className={`${size === "mobile" ? "text-[10px]" : "text-xs"} ml-1`}
          >
            Enter
          </span>
        </>
        ...
        <>
          <span
            className={`${size === "mobile" ? "text-[10px]" : "text-xs"} mr-1`}
          >
            Enter
          </span>
          <span className={size === "mobile" ? "text-base" : "text-lg"}>→</span>
...
export default function NkFloorPage({
  ...
  const [mobileCarouselPage, setMobileCarouselPage] = useState<1 | 2>(1);
  ...
  // 1. Retrieve User Session (Backend Logic)
  useEffect(() => {
  ...
  // 3. REDIRECT LOGIC (Backend Logic)
  useEffect(() => {
    if (!isValid) router.replace("/room/3/floor/1");
  }, [isValid, router]);

  // 4. MATCHING LOGIC (Backend Logic)
  ...
  async function handleConfirmBooking() {
    ...
    try {
      ...
      if (result.success) {
        setToast(
          `${fullRoomId} booked successfully! Room details sent to your email.`,
        );
  ...
  // Room card component - Updated with backend logic but keeping UI design
  function RoomCard({
    room,
    top,
    left,
    ...
    // UI colors based on status (from first code design)
    ...
    return (
      <button
        ...
        disabled={!isRoomActive || isFull || loading || isBooking}
        onClick={() => {
          // --- ONE ROOM PER USER CHECK --- (Backend Logic)
      ...
      >
        <div className="flex h-full flex-col items-center justify-center leading-tight p-1 sm:p-2">
          ...
          <span className="text-[8px] xs:text-[9px] sm:text-[10px] md:text-[11px] text-slate-700 whitespace-nowrap">
            {!isRoomActive
              ? roomInfo.disabledReason || null
              : isFull
                ? "Fully Booked"
                : `${occupied}/${capacity}`}
          </span>
  ...
  const leftRooms = useMemo(
    () => nkLeftRoomsForFloor(floorNum as 1 | 2 | 3 | 4),
    [floorNum],
  );
  const middleRooms = useMemo(
    () => nkMiddleRoomsForFloor(floorNum as 1 | 2 | 3 | 4),
    [floorNum],
  );
  const rightRooms = useMemo(
    () => nkRightRoomsForFloor(floorNum as 1 | 2 | 3 | 4),
    [floorNum],
  );
  const allRooms = useMemo(() => {
    const l = nkLeftRoomsForFloor(floorNum as 1 | 2 | 3 | 4);
    const m = nkMiddleRoomsForFloor(floorNum as 1 | 2 | 3 | 4);
    const r = nkRightRoomsForFloor(floorNum as 1 | 2 | 3 | 4);
    return [...l, ...m, ...r];
  }, [floorNum]);
      ...
      <div className="mx-auto max-w-full px-2 xs:px-3 sm:px-4 md:px-6 lg:px-8">
        ...
        {sidebarOpen && (
          ...
          >
            ...
            >
              <FloorSidebar
                currentFloor={floorNum as 1 | 2 | 3 | 4}
                baseHref="/room/3/floor"
              />
            </div>
          </div>
        )}
              ...
              <section className="relative w-full overflow-hidden rounded-xl sm:rounded-2xl lg:rounded-3xl border border-slate-200 bg-white/80 pt-1 px-3 sm:px-4 md:px-6 lg:px-8 xl:px-10 pb-3 sm:pb-4 md:pb-6 lg:pb-8 shadow-sm">
                ...
                <div className="relative aspect-[16/7] w-full">
                  ...
                  <div
                    ...
                    style={{
                      ...(isMid
                        ? midLayout.washroomRight
                        : desktopLayout.washroomLeft),
