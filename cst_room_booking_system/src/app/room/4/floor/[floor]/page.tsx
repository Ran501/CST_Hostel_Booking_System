...
export default function HaFloorPage({
  params,
}: {
  ...
  const leftRooms = useMemo(
    () => haLeftRoomsForFloor(floorNum as 1 | 2 | 3),
    [floorNum],
  );
  const rightRooms = useMemo(
    () => haRightRoomsForFloor(floorNum as 1 | 2 | 3),
    [floorNum],
  );
  ...
  const RoomBlock = ({ room }: { room: number }) => {
    ...
    const borderColor = !isRoomActive
    ...
    return (
      <button
        className={`group relative rounded-xl border shadow-sm transition-all duration-200 w-full h-full flex flex-col items-center justify-center p-1 ${borderColor}`}
      ...
      >
        ...
        {info?.forGender && (
          <span className="text-[7px] uppercase font-black text-slate-400 mt-0.5">
            {info.forGender}
          </span>
        )}
      </button>
    ...
    <main className="min-h-screen bg-zinc-100 py-4 sm:py-6 md:py-8 text-slate-900 overflow-x-hidden">
      <div className="mx-auto w-full max-w-full px-3 xs:px-4 sm:px-6 lg:max-w-7xl lg:px-8">
        ...
        <div className="mb-5 sm:mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          {currentUser && (
            <div className="text-xs font-semibold bg-white px-4 py-1.5 rounded-full border border-slate-200 shadow-sm flex items-center gap-2">
              <span>
                Booking as:{" "}
