...
// Define the shape of our Neon Database data
interface RoomData {
  roomNumber: string;
  occupied: number;
  capacity: number;
...
interface UserSession {
  ...
  role: string;
  gender: string;
  hasBooked?: boolean; // Track booking status
}
...
export default function RkbFloorPage({
  params,
}: {
  ...
  useEffect(() => {
    if (!isValid) {
      router.replace("/room/2/floor/1");
    }
  }, [isValid, router]);

  const leftRooms = useMemo(
    () => leftColumnRoomsForFloor(floorNum as 1 | 2 | 3 | 4),
    [floorNum],
  );
  const rightRooms = useMemo(
    () => rightColumnRoomsForFloor(floorNum as 1 | 2 | 3 | 4),
    [floorNum],
  );
  ...
  const RoomBlock = ({ room }: { room: number }) => {
    ...
    const isRoomActive =
      dbValue !== false && String(dbValue).toUpperCase().trim() !== "FALSE";
    ...
    const colors = !isRoomActive
      ...
      : isFully
        ...
        : isSelected
          ...
          : isPartial
            ? "border-amber-200 bg-amber-50 text-amber-700 hover:border-amber-300/80 hover:bg-amber-50/80 hover:-translate-y-0.5 hover:shadow-md active:scale-[0.98]"
  ...
  return (
    <main className="min-h-screen bg-zinc-100 py-4 sm:py-6 md:py-8 text-slate-900 overflow-x-hidden">
      <div className="mx-auto w-full max-w-full px-3 xs:px-4 sm:px-6 lg:max-w-7xl lg:px-8">
        ...
        {toast && (
          <div
            className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-[60] px-6 py-3 rounded-xl shadow-2xl flex items-center gap-3 border animate-in fade-in slide-in-from-bottom-4 duration-300 text-white ${
              /* This logic checks for keywords to switch from green to red */
              (toast.toLowerCase().includes("booked") &&
