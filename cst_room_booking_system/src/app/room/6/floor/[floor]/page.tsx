...
interface UserSession {
  ...
  role: string;
  gender: string;
  hasBooked?: boolean; // Track if user already has a reservation
}
...
export default function HcFloorPage({
  params,
}: {
  ...
  // 1. Retrieve User Session from LocalStorage on mount
  useEffect(() => {
  ...
  useEffect(() => {
    if (!isValid) {
      router.replace("/room/6/floor/1");
    }
  }, [isValid, router]);

  const leftRooms = useMemo(
    () => hcLeftRoomsForFloor(floorNum as 1 | 2 | 3),
    [floorNum],
  );
  const rightRooms = useMemo(
    () => hcRightRoomsForFloor(floorNum as 1 | 2 | 3),
    [floorNum],
  );
      ...
      <button
        ...
        onClick={() => {
          ...
          if (currentUser?.hasBooked) {
            setToast("You already have booked room.");
            setTimeout(() => setToast(null), 3000);
            return;
          }
                  ...
                  <div className="flex flex-col items-center gap-2 xs:gap-3 sm:gap-3 md:gap-4">
                    {(floorNum as 1 | 2 | 3) === 1 ? (
                      ...
                      <>
                        ...
                        {/* Enter sign for floors 2 & 3 - Visible on ALL screens */}
