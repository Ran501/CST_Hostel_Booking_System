// HB Hostel Building Data Configuration

export const HB_NAME = "HB";
export const HB_FLOORS = [1, 2, 3];

export function hbAllRoomsForFloor(floor) {
  const base = floor * 100;
  const count = floor === 1 ? 13 : 14; // Floor 1: 13 rooms, Floors 2-3: 14 rooms
  return Array.from({ length: count }, (_, i) => base + i + 1);
}

export function hbLeftRoomsForFloor(floor) {
  const b = floor * 100;
  if (floor === 1) {
...
export function hbRightRoomsForFloor(floor) {
  const b = floor * 100;
  if (floor === 1) {
...
export function hbIsRoomNumberOnFloor(room, floor) {
  const base = floor * 100;
  const max = floor === 1 ? base + 13 : base + 14;
  return room >= base + 1 && room <= max;
}

export function hbBookedRoomsForFloor(floor) {
