...
const RoomBlock = ({ room, selectedRoom, setSelectedRoom }) => {
...
export default function HostelFloorPage({ params }) {
  const router = useRouter();
  const { hostel, floor } = use(params);
  
  // Validate and get hostel config
  const config = getHostelConfig(hostel);
  if (!config) {
    return <div>Hostel not found</div>;
  }

  const hostelName = config.name;
  ...
  // Get rooms based on hostel layout
  const isThreeColumnLayout = config.layout === '3-column';
  
  const leftRooms = useMemo(
    () => config.leftFunc(floorNum),
    [floorNum]
  );
  const middleRooms = useMemo(
    () => isThreeColumnLayout ? config.middleFunc(floorNum) : [],
    [floorNum, isThreeColumnLayout]
  );
  const rightRooms = useMemo(
    () => config.rightFunc(floorNum),
    [floorNum]
  );

  function handleConfirmBooking() {
    if (selectedRoom === null) return;
    alert(`Room ${hostelName}-${selectedRoom} booking confirmed! (UI Only - No Backend)`);
      ...
      <div className="mx-auto w-full max-w-full px-3 xs:px-4 sm:px-6 lg:max-w-7xl lg:px-8">
        ...
        <div className="md:hidden flex items-center justify-between mb-4">
          ...
          <h1 className="text-center text-base xs:text-lg font-semibold tracking-wide flex-1">
            {hostelName} {floorLabel(floorNum)} floor
          </h1>
          ...
          <div className="flex flex-col md:flex-row gap-4 lg:gap-6">
            ...
            <div className="flex-1 min-w-0">
              <section className="relative rounded-xl sm:rounded-2xl border border-slate-200 bg-white/80 p-3 sm:p-4 md:p-6 shadow-lg lg:shadow-xl backdrop-blur overflow-hidden w-full">
                ...
                {/* 3-column layout for NK */}
                {isThreeColumnLayout && (
                  <div className="grid grid-cols-[1fr_auto_1fr_auto_1fr] gap-2 xs:gap-3 sm:gap-4 md:gap-5 lg:gap-6 pt-16 xs:pt-18 sm:pt-20 md:pt-24">
                    <div className="flex flex-col items-center gap-2 xs:gap-3 md:gap-4">
                      {leftRooms.map((r) => (
                        <div
                          key={r}
              ...
              {/* RoomLegend - Single import, reused for all hostels */}
        ...
        {selectedRoom !== null && (
          <ConfirmationDialog
            message={`Do you want to book one bed from Room ${hostelName}-${selectedRoom}?`}
            isLoading={false}
