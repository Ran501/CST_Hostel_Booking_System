...
// // ── CSV parser (browser-side, for preview only) ────────────────────────────
...
//     const studentId = v[idx("student id")] || v[idx("studentid")] || v[idx("student_id")] || "";
...
//   const COLS = ["Student ID", "Name", "Email", "Department", "Year", "Phone", "Gender"];
...
//   const allRows = [COLS, ...students.map((s) => [s.studentId, s.name, s.email, s.department, s.year, s.phone, s.gender])];
//   const allWs = XLSX.utils.aoa_to_sheet(allRows);
//   styleSheet(allWs, allRows.length, COLS.length);
...
//     .forEach(([dept, list]) => {
//       const rows = [COLS, ...list.map((s) => [s.studentId, s.name, s.email, s.department, s.year, s.phone, s.gender])];
//       const ws = XLSX.utils.aoa_to_sheet(rows);
//       styleSheet(ws, rows.length, COLS.length);
...
//   const [cursors, setCursors] = useState([null]); // stack: cursors[0]=null=page1, cursors[1]=cursor after page1, etc.
//   const [currentPage, setCurrentPage] = useState(0); // index into cursors stack
...
//     const newCursors = [...cursors.slice(0, newPage + 1)]; // trim forward history
...
//       try {
//         const parsed = parseCSV(ev.target.result);
//         if (parsed.length === 0) { alert("No valid students found in CSV"); setImporting(false); return; }
...
//         setCursors([null]);
//         setCurrentPage(0);
//         alert(`Successfully imported ${result.count} students! (${result.skipped} skipped as duplicates)`);
//       } catch (err) {
...
//                         <button onClick={() => handleEdit(student)} className="text-blue-600 hover:text-blue-800 cursor-pointer">
...
//                 Showing <span className="font-medium">{pageStart}–{pageEnd}</span> of{" "}
...
//                 {[
//                   { label: "Full Name", key: "name", type: "text", placeholder: "Enter full name" },
//                   { label: "Student ID", key: "studentId", type: "text", placeholder: "Enter student ID" },
//                   { label: "Email Address", key: "email", type: "email", placeholder: "Enter email address" },
//                   { label: "Phone Number", key: "phone", type: "text", placeholder: "Enter phone number" },
//                 ].map(({ label, key, type, placeholder }) => (
//                   <div className="mb-4" key={key}>
...
//                       onChange={(e) => setNewStudentData({ ...newStudentData, [key]: e.target.value })}
//                       placeholder={placeholder}
...
//                       className="w-full appearance-none bg-white border border-gray-300 rounded-lg px-4 py-2 pr-10 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
//                     >
//                       {DEPARTMENTS.filter((d) => d !== "All").map((dept) => (
//                         <option key={dept} value={dept}>{dept}</option>
//                       ))}
...
//                           <thead className="bg-gray-50">
//                             <tr>
//                               {["Name", "Student ID", "Email", "Department", "Year", "Phone", "Gender"].map((col) => (
//                                 <th key={col} className="border-b px-3 py-2 text-left">{col}</th>
//                               ))}
//                             </tr>
//                           </thead>
//                           <tbody>
...
//                       <strong>Note:</strong> Duplicate student numbers and emails will be skipped automatically.
//                     </p>
//                   </div>
//                 </div>
